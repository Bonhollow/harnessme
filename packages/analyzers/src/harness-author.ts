...
function validateAuthoredMarkdown(
  ...
  for (const section of requiredSections) {
    if (!normalized.includes(section)) throw new Error(`AI-authored AGENTS.md is missing required section: ${section}`);
  }
  ...
  if (!requiredTokens.every((token) => occurrences(normalized, token) === 1)) {
    throw new Error("AI-authored AGENTS.md must contain each HarnessME managed placeholder exactly once.");
  }
  if (!/before editing[^\n]*ask the developer for explicit confirmation/iu.test(normalized)) {
    throw new Error("AI-authored AGENTS.md omitted the explicit pre-edit developer confirmation rule.");
  }
  if (/<!--\s*Generated by HarnessME|HARNESSME:PENDING:/u.test(normalized)) {
    throw new Error("AI-authored AGENTS.md attempted to write HarnessME-managed markers.");
  }
  const unknownTokens = normalized.match(/\{\{HARNESSME_[A-Z_]+\}\}/gu)?.filter((token) => !requiredTokens.includes(token)) ?? [];
  if (unknownTokens.length) throw new Error(`AI-authored AGENTS.md contains unknown managed placeholders: ${unknownTokens.join(", ")}`);
  for (const gate of gates) {
    if (!eligible.has(gate.path)) throw new Error(`AI selected an unverified critical path: ${gate.path}`);
  }
  for (const language of facts.stack.languages) {
    if (!normalized.includes(language.name)) throw new Error(`AI-authored AGENTS.md omitted detected language: ${language.name}`);
  }
  for (const framework of facts.stack.frameworks) {
    if (!normalized.includes(framework)) throw new Error(`AI-authored AGENTS.md omitted detected framework: ${framework}`);
  }
  for (const command of analysis.commands) {
    if (!normalized.includes(command)) throw new Error(`AI-authored AGENTS.md omitted validated command: ${command}`);
  }
  ...
  for (const convention of facts.conventions.facts) {
    ...
    if (!cited) throw new Error(`AI-authored AGENTS.md omitted evidence for convention: ${convention.statement}`);
  }
  return `${normalized}\n`;
}
...
Keep all required headings and exactly one each of ${CRITICAL_PATHS_TOKEN}, ${VERIFIED_CHANGES_TOKEN}, ${DIRECTIVES_TOKEN}, and ${PENDING_TOKEN}. The critical-path section must explicitly require the coding agent to stop and ask the developer for explicit confirmation before editing a listed path, and must prohibit self-approval or bypass. Return a short comparison summary alongside the corrected Markdown and final gates.`;
  ...
  return {
    markdown: validateAuthoredMarkdown(reviewed.markdown, reviewed.gates, eligible, options.facts, options.analysis),
    gates: [...new Map(reviewed.gates.map((gate) => [gate.path, gate])).values()],
