...
const DraftSchema = z.object({
  ...
  references: z.array(ReferenceDocumentSchema).max(12).default([]),
...
function validateFeatures(features: FeatureDefinition[], analysis: AnalysisResult): FeatureDefinition[] {
  ...
  const paths = new Set([...analysis.sourceFiles, ...analysis.scopePaths ?? [], ...analysis.stack.documentationPaths ?? []]);
  ...
  for (const feature of features) {
    ...
    for (const scope of feature.scopes) {
      ...
      if (![...analysis.sourceFiles, ...analysis.scopePaths ?? []].some((path) => path === base || path.startsWith(`${base}/`) || (scope.includes("*") && minimatch(path, scope, { dot: true })))) invalid(`AI-authored feature ${feature.slug} uses an unverified scope: ${scope}`);
    }
...
function normalizeFeatureScopes(features: FeatureDefinition[], analysis: AnalysisResult): FeatureDefinition[] {
  return features.map((feature) => ({
    ...
    scopes: feature.scopes.filter((scope) => {
      const base = scope.replace(/\*.*$/u, "").replace(/\/$/u, "");
      const matchesSource = [...analysis.sourceFiles, ...analysis.scopePaths ?? []].some((path) => path === base || path.startsWith(`${base}/`) || (scope.includes("*") && minimatch(path, scope, { dot: true })));
...
/** Retain only validated extra files that AI guidance cites, so graph facts stay bounded. */
export function citedScopePaths(analysis: AnalysisResult, features: FeatureDefinition[], references: ReferenceDocument[]): string[] {
  const available = new Set(analysis.scopePaths ?? []);
  const indexed = new Set([...(analysis.structure?.files ?? []).map((item) => item.path), ...(analysis.structure?.documents ?? [])]);
  const cited = [
    ...features.flatMap((feature) => [...feature.citations, ...feature.relationships.flatMap((relation) => relation.citations)].map((item) => item.path)),
    ...references.flatMap((reference) => [...reference.markdown.matchAll(/`([^`\n]+):\d+`/gu)].map((match) => match[1]!)),
  ];
  const discovered = [...available].filter((path) => !path.split("/").at(-1)?.startsWith(".")).sort();
  for (const feature of features) for (const scope of feature.scopes) {
    let count = 0;
    for (const path of discovered) {
      if (!minimatch(path, scope, { dot: true }) && path !== scope && !path.startsWith(`${scope.replace(/\*.*$/u, "").replace(/\/$/u, "")}/`)) continue;
      cited.push(path);
      if (++count === 3) break;
    }
  }
  return [...new Set(cited)].filter((path) => available.has(path) && !indexed.has(path)).sort();
}
...
function normalizeReferenceScope(reference: ReferenceDocument, analysis: AnalysisResult, facts: FactsSnapshot): ReferenceDocument {
  const isGrounded = (scope: string): boolean => {
    const base = scope.replace(/\*.*$/u, "").replace(/\/$/u, "");
    const paths = [...analysis.sourceFiles, ...analysis.scopePaths ?? [], ...facts.stack.documentationPaths ?? []];
...
function validateAuthoredMarkdown(
  ...
  let normalized = normalizeHeadingAliases(markdown.trim())
    ...
    .trim();
  const detectedDocuments = facts.stack.documentationPaths ?? [];
  const routesToDocument = (path: string): boolean => normalized.includes(`\`${path}\``) || normalized.includes(`](${path})`);
  if (detectedDocuments.length && !detectedDocuments.some(routesToDocument)) {
    normalized = normalized.replace("## Before editing\n", `## Before editing\n\n- Read \`${detectedDocuments[0]}\` when the task needs repository context.\n`);
  }
  ...
  const groundedPath = (value: string): boolean => {
    ...
    return eligible.has(path)
      || eligible.has(`${path}/**`)
      || [...analysis.sourceFiles, ...analysis.scopePaths ?? []].some((source) => source === path || source.startsWith(`${path}/`));
  };
  ...
  const documentationPaths = detectedDocuments;
  const documentationMaintenance = sectionBody(normalized, "## Documentation maintenance");
  if (documentationPaths.length && !documentationPaths.some(routesToDocument)) {
