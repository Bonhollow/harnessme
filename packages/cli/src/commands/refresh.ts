...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    const { result, quality, conflictCount, progress } = await stageRepositoryMutation(project, async (root) => {
      ...
      return { result, quality, conflictCount: analysis.documentationConflicts?.length ?? 0, progress };
    });
    ...
    if (conflictCount) info(`Documentation conflicts requiring review: ${conflictCount}.`);
  },
});
