...
export function assessHarnessQuality(
  ...
  const deepReferences = references.length > 0 && references.every((reference) => [
    "Scope",
    "Responsibilities",
    "Extension seams",
    "Invariants",
  ...
  ].every((heading) => has(reference.markdown, heading)));
  const checks = [
    ...
    { id: "reference-pack", passed: deepReferences, points: 10, message: "Scoped references provide extension seams, invariants, impact maps, anti-patterns, workflows, validation, and maintenance triggers." },
